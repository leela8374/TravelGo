...
from models.db import trains_col, bookings_col, users_col
...
def search_trains():
    ...
    results = list(trains_col.find(query, {"_id": 0}))
    return jsonify({"results": results, "type": "train"}), 200


@trains_bp.route("/book", methods=["POST"])
@jwt_required()
def book_train():
    ...
    return jsonify({"message": "Train booked successfully!", "booking": booking, "booking_ref": ref}), 201
